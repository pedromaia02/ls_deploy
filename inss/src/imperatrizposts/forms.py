...
class PostForm(forms.ModelForm):
	...
	PROFESSIONAL_NOMES = []
	nomes = [nome.encode("utf8") for nome in ImperatrizProfissionals.objects.all().values_list('nome', flat=True)]
	for nome in nomes:
	...
	LOCAIS = []
	nomes = [nome.encode("utf8") for nome in ImperatrizCidades.objects.all().values_list('nome', flat=True)]
	for nome in nomes:
