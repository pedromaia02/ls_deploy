...
from .models import ImperatrizPosts,ImperatrizProfissionals,ImperatrizCidades
...
def post_create(request):
	...
	if form.is_valid():
		...
		return redirect("imperatrizposts:list")

	context = {
		"form":form,
	}
	return render(request,"post_imperatriz_form.html", context)

#@login_required(login_url='/login/')
def post_detail(request, id=None):
	instance = get_object_or_404( ImperatrizPosts,id=id)
	...
	profissional_cargos = ImperatrizProfissionals.objects.filter(nome__in=profissional_nomes).values_list('nome','cargo')
	...
	return render(request,"post_imperatriz_detail.html", context)

#@login_required(login_url='/login/')
def post_list(request):

	queryset = ImperatrizPosts.objects.all().order_by('-data')
	query = request.GET
	...
	context = {
			...
			"table": table,
			"cidades":ImperatrizCidades.objects.all().order_by('nome').values_list('nome', flat=True)
	 	}
	return render(request,"post_imperatriz_list.html", context)
