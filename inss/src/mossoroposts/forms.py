...
class PostForm(forms.ModelForm):
	...
	PROFESSIONAL_NOMES = []
	nomes = [nome.encode("utf8") for nome in MossoroProfissionals.objects.all().values_list('nome', flat=True)]
	for nome in nomes:
	...
	LOCAIS = []
	nomes = [nome.encode("utf8") for nome in MossoroCidades.objects.all().values_list('nome', flat=True)]
	for nome in nomes:
