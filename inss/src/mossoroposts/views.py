...
from .models import MossoroPosts,MossoroProfissionals,MossoroCidades
...
def post_create(request):
	...
	if form.is_valid():
		...
		return redirect("mossoroposts:list")

	context = {
		"form":form,
	}
	return render(request,"post_mossoro_form.html", context)

#@login_required(login_url='/login/')
def post_detail(request, id=None):
	instance = get_object_or_404( MossoroPosts,id=id)
	...
	profissional_cargos = MossoroProfissionals.objects.filter(nome__in=profissional_nomes).values_list('nome','cargo')
	...
	return render(request,"post_mossoro_detail.html", context)

#@login_required(login_url='/login/')
def post_list(request):

	queryset = MossoroPosts.objects.all().order_by('-data')
	query = request.GET
	...
	context = {
			...
			"table": table,
			"cidades":MossoroCidades.objects.all().order_by('nome').values_list('nome', flat=True)
	 	}
	return render(request,"post_mossoro_list.html", context)
