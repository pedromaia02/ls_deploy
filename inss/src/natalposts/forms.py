...
class PostForm(forms.ModelForm):
	...
	PROFESSIONAL_NOMES = []
	nomes = [nome.encode("utf8") for nome in NatalProfissionals.objects.all().values_list('nome', flat=True)]
	for nome in nomes:
	...
	LOCAIS = []
	nomes = [nome.encode("utf8") for nome in NatalCidades.objects.all().values_list('nome', flat=True)]
	for nome in nomes:
